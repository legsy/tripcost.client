      ...
      <template v-slot:top>
        <v-layout wrap>
          <v-flex xs12 sm12 md4 mt-3 pl-4>
            <div class="text-left">
              <v-toolbar-title>{{ $t('trips.TITLE') }}</v-toolbar-title>
            </div>
          </v-flex>
...
export default {
  metaInfo() {
    return {
      ...
      titleTemplate: `${this.$t('trips.TITLE')} - %s`
    }
  },
  data() {
    return {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        {
          text: this.$i18n.t('trips.headers.NAME'),
          align: 'left',
          sortable: true,
          value: 'name'
        },
    ...
    items() {
      return this.$store.state.trips.items
    },
    totalItems() {
      return this.$store.state.trips.totalItems
    }
  },
  watch: {
    dialog(value) {
  ...
  methods: {
    ...mapActions(['getTrips', 'editTrip', 'saveTrip', 'deleteTrip']),
    getFormat(date) {
    ...
    async deleteItem(item) {
      try {
        ...
        if (response) {
          this.dataTableLoading = true
          await this.deleteTrip(item._id)
          await this.getTrips(
            buildPayloadPagination(this.pagination, this.buildSearch())
          )
          this.dataTableLoading = false
      ...
      if (isValid) {
        try {
          ...
          if (this.editedItem._id) {
            await this.editTrip(this.editedItem)
            await this.getTrips(
              buildPayloadPagination(this.pagination, this.buildSearch())
            )
            this.dataTableLoading = false
            ...
            // Creating new item
            await this.saveTrip({ name: this.editedItem.name })
            await this.getTrips(
              buildPayloadPagination(this.pagination, this.buildSearch())
            )
            this.dataTableLoading = false
